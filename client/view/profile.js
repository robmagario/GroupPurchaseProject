...
Template.ProfilePage2.rendered = function() {
};

Template.ProfilePage2.events({
...
Template.ProfilePage2.helpers({
    ...
    'UploadProfilePoster': function() {
        return {
            finished: function (index, fileInfo, context) {
                var _oldurl = $('#UserPoster').attr('src');
                ...
                var _icon_old = ProfilePosters.findOne({user:Meteor.userId()});
                if(_icon_old != null) {
                    ProfilePosters.update({
                        _id: _icon_old._id
                    }, {$set:{
